...
export default defineSchema({
  ...
  trucks: defineTable({
    ...
    schedule: v.object({ days: v.array(v.string()), times: v.array(v.string()) }), // Example: {days: ['Mon', 'Tue'], times: ['10AM-6PM']}
  ...
  stands: defineTable({
    ...
    menu_id: v.string(),
    open_status: v.boolean(),
    schedule: v.object({ days: v.array(v.string()), times: v.array(v.string()) }),
  }).index("by_vendor", ["vendor_id"]),
